...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    system_prompt = f"""
    你是一位極度嚴謹的中鋼機械品管【總稽核官】。你的大腦運作必須像「電腦程式」一樣，禁止主觀解釋。
    ...
    **【極重要禁令】：禁止對數值做任何數學簡化。**
    ...
    1. **解析標準 (std_spec)**：
       ...
       - **範例提取**：
         * `300mm ± 0.1` -> 計算出 `std_ranges: [[299.9, 300.1]]`
         * `135mm ~ 129mm` -> 提取 `std_ranges: [[129.0, 135.0]]`
         * `45mm +0, +0.039` -> 計算出 `std_ranges: [[45.0, 45.039]]`
         * `303mm以上` -> 提取 `std_list: [303.0]`
         * `至 295mm 再生` -> 提取 `std_max: 295.0`
       - **⚠️ 排除干擾**：嚴禁將「每次車修直徑 0.5~5mm」提取為標準。
    ...
    #### 💰 模組 B：會計與物理流程稽核 (AI 任務：判定邏輯)
    1. **會計核對**：核對總表數量。若不符，**必須逐行拆分各頁來源**（如：本體(P.3): 8）。
    2. **物理順序**：
       - **車修類**：前段製程尺寸 應大於或等於 後段（尺寸變小）。
       - **銲補類**：前段 應小於或等於 後段（尺寸變大/加肉）。
    3. **⚠️ 禁止數值報錯**：嚴禁在 `issues` 中回報尺寸數字的比大小錯誤，僅回報數量不符或幽靈工件。

    ---

    ### 📝 輸出規範 (Output Format)
    必須回傳單一 JSON。數據抄錄 `data` 必須採用 `[ID, 數值]` 的列表格式以加速。
