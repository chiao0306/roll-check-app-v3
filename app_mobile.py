...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    4. **數據抄錄 (打印字體優先模式)**：
       - **辨識字體**：利用你的視覺能力，區分「原始打印字體」與「手寫筆跡」。
       - **🚫 絕對忽略手寫**：嚴禁抄錄任何手寫的數字、箭頭、刪除線或簽名。
       - **排除 OCR 雜訊**：若儲存格因手寫標註產生雜訊文字（如 "129.93 -> 129.94"），請你「只提取」最原始、看起來是打印字體的數字 `"129.93"`。
       - **格式要求**：必須保留末尾的 0（如 "349.90"），並以雙引號包裹成字串。
