...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    3. **項目分類識別與排除邏輯 (極重要)**：
       ...
       - **精加工觸發條件**：只有在標題中「完全不含未再生」，且包含「再生、研磨、精加工、車修加工」等字眼時，才歸類為 `range`。
       - **銲補特權**：只要標題含「銲補」或「Welding」，分類「必須」設為 `銲補`。**嚴禁**因為標題含軸頸或未再生而將其改為其他分類。
       - **軸頸優先**：若非銲補，但標題含「軸頸」且含「未再生」，設為 `軸頸未再生`。
       - **本體優先**：若標題含「未再生」且不含「軸頸」，設為 `未再生本體`。
