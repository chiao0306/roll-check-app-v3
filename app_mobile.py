...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    2. **總表核對 (Global Check)**：
       - **聚合模式 (Aggregated)**：標題含「機ROLL車修/銲補/拆裝」，執行 Sum(本體+軸頸)。
       - **標準模式 (Standard)**：其餘項目僅加總名稱對應的子項。
    3. **物理順序與依賴**：車修類應「前段 >= 後段」；銲補類應「前段 <= 後段」。
    4. **⚠️ 禁止數值報錯**：嚴禁在 `issues` 中回報關於尺寸數字的大小錯誤，僅回報數量不符、流程中斷或幽靈工件。
            ...
            try:
                ...
                if cat == "未再生本體":
                    ...
                    if val <= t_used:
                        ...
                        if not is_two_dec: is_passed, reason = False, f"未再生(>標準{t_used}): 應填兩位小數"
                ...
                # --- 3. 精加工再生類 (公差區間 + 兩位小數) ---
                    ...
                    if not is_two_dec:
                        ...
                        if not any(r[0] <= val <= r[1] for r in s_ranges if len(r)==2):
                            is_passed, reason = False, f"不在規範區間內 {s_ranges}"
                    elif clean_std:
                        # 保底：若 AI 沒算出區間，檢查是否在最大最小數字之間
                        s_min, s_max = min(clean_std), max(clean_std)
                        if not (s_min <= val <= s_max):
                            is_passed, reason = False, f"不在區間內 {s_min}~{s_max}"
                ...
                # --- 4. 銲補 (整數 + mm 基準就近匹配) ---
                    ...
                    if not is_pure_int:
                        ...
                        if val < t_used: is_passed, reason = False, f"銲補不足: 實測 {val} < 標準 {t_used}"
