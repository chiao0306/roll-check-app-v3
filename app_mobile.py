...
def agent_unified_check(combined_input, full_text_for_search, api_key, model_name):
    ...
    2. **目標規格解析 (mm 定位與雜訊過濾)**：
       ...
       - **🚫 停止預算 (提速關鍵)**：若規格含 `±` 或偏差（如 +0.1/-0.2），**嚴禁在此執行數學運算**。請保持 `ranges_list` 為空，並將含 mm 及其公差的原始文字完整填入 `std_spec` 欄位即可。
    ...
    3. **項目分類決策流程 (由上至下執行，命中即停止)**：
       ...
       - **LEVEL 3：精加工與裝配判定**
         ...
         * (💡 提示：這類項目要求兩位小數，規格多以「區間」或「± 公差」呈現，僅需精確抄錄規格文字，不需預算結果)。
    ...
    5. **尺寸大小邏輯檢查**：
       ...
       - **判定要求**：針對同一 Roll ID，跨製程之尺寸大小必須符合上述位階邏輯。注意：同一編號出現在不同項目表格中代表「全流程紀錄」，屬於正常現象，嚴禁判定為衝突。 僅在位階不符（例如：研磨尺寸大於再生車修）時，才回報 🛑流程異常。
...
def python_accounting_audit(dimension_data, res_main):
    ...
    for item in dimension_data:
        ...
        if "1SET=4PCS" in u_local: 
            ...
            actual_item_qty = len(set(ids)) # 本體/強制去重
        else: 
    ...
    for s_title, data in global_sum_tracker.items():
        if data["actual"] != data["target"] and data["target"] > 0:
            ...
            i_type = "統計不符(運費)" if "運費" in s_title else "統計不符(總帳)"
